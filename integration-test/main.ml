...
let () =
  assert (Array.equal Int.equal (Array.initialize 5 ~f:(fun _ -> 0)) [|0;0;0;0;0|]);
  ...
  assert (List.equal Int.equal (List.initialize 5 ~f:(fun _ -> 0)) [0;0;0;0;0]);
  ...
  assert (Tuple3.equal Int.equal Int.equal Int.equal (Tuple3.make 1 1 1) (1, 1, 1));
