...
let suite =
  suite("Option", () => {
    ...
    describe("andThen2", () => {
      test(
        "Take 2 inputs and a function which returns a Some type if both are of Some type",
        () => {
        expect(
          Option.andThen2(
      ...
      test(
        "Take 2 inputs and a function which returns a None type if both are of None type",
        () => {
        expect(
          Option.andThen2(
            ~f=(a, b) => Some(Int.add(a, b)),
      ...
      test(
        ...
        () => {
        expect(
          Option.andThen2(~f=(a, b) => Some(Int.add(a, b)), None, None),
        )
        |> toEqual(Eq.(option(int)), None)
      });
    });
