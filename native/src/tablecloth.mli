...
module Tuple2 : sig
  ...
  *)
  ...
  (** Transform both values of a tuple using the same function. This function can only be used on tuples which have the same type for each value.
...
module Tuple3 : sig
  ...
  (** Create a 3-tuple.
    ...
    let zip3 (xs : 'a list) (ys : 'b list) (zs : 'c list) : ('a * 'b * 'c) list =
      List.map3 Tuple3.create xs ys zs
  *)
  ...
  *)
  ...
  (** Transform all the values of a tuple using the same function. This function can only be used on tuples which have the same type for each value.    
