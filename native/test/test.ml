...
let t_List () =
  ...
  AT.check (AT.option AT.int) "maximumBy empty list" (List.maximumBy ~f:(fun x -> x + 1) []) None;
  AT.check (AT.option AT.int) "maximumBy one element" (List.maximumBy  ~f:(fun x -> x + 1) [1]) (Some 1);
